...
RESEARCHER_DIR="$(cd "$MODEL_TEST_DIR/../.." && pwd)"
GITOPEDIA_DIR="$(cd "$RESEARCHER_DIR/../gitopedia" && pwd)"
OUTPUT_DIR="$MODEL_TEST_DIR/test_output"
...
cd "$RESEARCHER_DIR"
go build -o "$RESEARCHER_DIR/bin/test_summarization" "$MODEL_TEST_DIR/test_summarization/main.go"
